...
function animate() {
  ...
  const hoursin12 = date.getHours() >=12 ? date.getHours()-12 : date.getHours();
  const digtime = hoursin12 + ":" +date.getMinutes() + ampm;
  const hour = date.getHours() + date.getMinutes() / 60;
  const minute = date.getMinutes() + date.getSeconds() / 60;
  const second = date.getSeconds() + date.getMilliseconds() / 1000;

  textElement.textContent = showDate ? day : digtime;
